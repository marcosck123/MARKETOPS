...
export function PdvClient({ products, cashSession, promotions, companyName, slogan }: Props) {
  ...
  // NF modal — opens after successful payment
  const [nfSale, setNfSale] = useState<{ saleId: string; saleCode: string } | null>(null);
  ...
  return (
    <>
      ...
      <PaymentModal
        ...
        onSuccess={(sale) => { setActiveModal(null); setNfSale(sale); }}
        onClose={() => setActiveModal(null)}
      />

      {/* NF modal — after payment */}
      <NfModal
        open={nfSale !== null}
        saleId={nfSale?.saleId ?? ""}
        saleCode={nfSale?.saleCode ?? ""}
        operatorId={cashSession.operatorId}
        onClose={() => { cart.clearCart(); setNfSale(null); }}
      />

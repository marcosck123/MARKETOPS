      ...
      <section className="grid gap-4 xl:grid-cols-[1fr_360px]">
        <div className="rounded-lg border border-stone-200 bg-white shadow-sm">
          <div className="flex flex-col justify-between gap-4 border-b border-stone-200 px-5 py-4 xl:flex-row xl:items-center">
            <div>
              <h2 className="text-base font-semibold text-stone-950">Trilha de eventos</h2>
                  ...
                  {filteredLogs.map((log) => (
                    <tr key={log.id} className="text-stone-700">
                      <td className="px-5 py-4">
                        <p className="font-mono text-xs font-semibold text-stone-900">{log.id.slice(-8).toUpperCase()}</p>
                        <p className="mt-1 text-xs text-stone-500">{log.createdAt}</p>
                        <p className="mt-1 max-w-md text-xs text-stone-500">{log.description}</p>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-5 py-4">
                        <p className="font-medium text-stone-950">{log.actorName}</p>
                        <p className="mt-1 text-xs text-stone-500">{log.actorRole}</p>
                      </td>
                      <td className="whitespace-nowrap px-5 py-4">
                        <StatusBadge
...
function PolicyCard({ icon, title, description }: { icon: ReactNode; title: string; description: string }) {
  return (
    <article className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3 text-amber-700">{icon}</div>
  ...
  if (!log) {
    return (
      <aside className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
        <p className="text-sm text-stone-500">Nenhum log selecionado.</p>
      </aside>
    );
  }
    ...
    <aside className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 font-mono text-lg font-semibold text-stone-950">
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-stone-950/50 px-4 py-6">
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-amber-700">Evento manual</p>
            <h2 id="audit-event-modal-title" className="mt-1 text-xl font-semibold text-stone-950">
              Registrar log
            </h2>
          </div>
